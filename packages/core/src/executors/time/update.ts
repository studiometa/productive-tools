...
export async function updateTimeEntry(
  ...
  const data: { time?: number; billable_time?: number; note?: string; date?: string } = {};
  if (options.time !== undefined) data.time = options.time;
  if (options.billable_time !== undefined) data.billable_time = options.billable_time;
  ...
  if (Object.keys(data).length === 0) {
    throw new ExecutorValidationError(
      'No updates specified. Provide at least one of: time, billable_time, date, note',
      'options',
    );
  }
